...
def get_state_dict(ckpt_path: str, state_dict: Optional[Literal["epoch", "model", "optimizer", "optimizer_aux", "scaler"]] = "model"):
    ckpt = torch.load(ckpt_path, map_location='cpu')
    if state_dict == "model":
        if "state_dict" not in ckpt:
            raise KeyError(f"'state_dict' not found in checkpoint at {ckpt_path}")
        sd = ckpt["state_dict"]

    elif state_dict == "optimizer":
        if "optimizer" not in ckpt:
            raise KeyError(f"'optimizer' not found in checkpoint at {ckpt_path}")
        sd = ckpt["optimizer"]

    elif state_dict == "optimizer_aux":
        if "optimizer_aux" not in ckpt:
            raise KeyError(f"'optimizer_aux' not found in checkpoint at {ckpt_path}")
        sd = ckpt["optimizer_aux"]

    elif state_dict == "scaler":
        if "scaler" not in ckpt:
            raise KeyError(f"'scaler' not found in checkpoint at {ckpt_path}")
        sd = ckpt["scaler"]

    elif state_dict == "epoch":
        if "epoch" not in ckpt:
            raise KeyError(f"'epoch' not found in checkpoint at {ckpt_path}")
        sd = ckpt["epoch"]
    else:
        raise ValueError(f"Invalid state_dict type: {state_dict}. Must be 'epoch', 'model' or 'optimizer_aux' 'optimizer' or 'scaler'.")
